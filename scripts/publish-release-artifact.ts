...
const ARTIFACT_DIR = path.resolve(process.env.RELEASE_ARTIFACT_DIR ?? 'release-artifact')

/**
 * @param {string} output
 * @returns {boolean}
 */
function isMissingPublishedPackage(output: string): boolean {
...
const trustedPackage = JSON.parse(await fs.readFile(path.resolve('package.json'), 'utf8'))
